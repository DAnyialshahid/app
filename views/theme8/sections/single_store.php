...
<div  class="pt-4">
  ...
  <div class="row popular_stores d-flex justify-content-center"> 
    ...
    if (isset($data->store)){
      foreach (array_slice($data->store->coupons,0,9999999) as $voucher ) { 
    ...
    ?>
          <div class="store col-md-12 col-sm-12 col-12 ">
                      <a target="_blank"
                         id="<?=$voucher->id?>" 
                       href="?id=<?=$voucher->id?>"
                         locations="<?= !empty($voucher->affiliate_link)?$voucher->affiliate_link:$voucher->store->affiliate_link ?>"
                           onclick="go(this)"
                        >
                         <p align="center" ><?=$voucher->name?> </p>
                      </a>
                   </div>
         ...
         <div class="col-md-12 store-flex-wrapper">
            ...
            <div class="store-offers-col" >
              ...
              <div class="store-profile-col">
                    ...
                    <h3 class="text-center" >{{row.name}} <div></div>Coupons {{new Date().toLocaleString('default', { month: 'long' })+' ' + new Date().getFullYear() }}</h3> </div>
