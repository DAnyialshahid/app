    ...
    return {
        ...
        submit: function(redirect_url=null) {
            ...
            defaultForm.validate().then(function(status) {
                if(status=='Valid'){
                   var url= 'createCategoryGroup';
                   var title= 'Category Created';
                     if($('#id').val()!=''){
                        url="updateCategoryGroup";
      ...
      return false;
        ...
        fillUpdate: function(id) {  
             ...
             jQuery.ajax({
                     type : "post",
                     dataType : "json",
                     url : api_base_url+"/getCategoryGroup/"+id,
                     ...
                     success: function(data) { 
                        ...
                        if(data.success === "yes" ) { 
                          var row=data.response;
                                        ...
                                        $('[name=seo_category_group_meta_title]').val(row.seo_category_group_meta_title);
                                        $('[name=seo_category_group_meta_keywords]').val(row.seo_category_group_meta_keywords);
                                        ...
                                        $('[name=seo_category_group_meta_description]').val(row.seo_category_group_meta_description);
...
$(document).ready(function() {
  ...
  Main.fillUpdate($('#id').val());
       $('#card_label').html('Update Category Group');

        $('#submit_btn_1').html(' Update  & exit');
        $('#submit_btn_2').html('Update');
    }else{

        $('#card_label').html('Create Category Group');
